...
async def submit_answer(
    ...
    if request.is_correct:
        ...
        if review.review_count == 1:
            ...
            # Subsequent correct answers: multiply previous interval by ease factor
            current_interval = review.interval_days or 1  # Fallback to 1 if None
            ease = review.ease_factor / 100.0
            review.interval_days = max(1, int(current_interval * ease))
            
    else:
        # Reset on failure but allow classification system to handle status
        review.success_streak = 0
        review.is_difficult = True
        review.interval_days = 1  # Reset interval to 1 day
        
        # Adjust ease factor down
        review.ease_factor = max(review.ease_factor - 20, 130)  # Floor at 1.3
    
    # Calculate next due date with special handling for isolation phase
    interval_days = review.interval_days or 1  # Fallback to 1 if still None
    ease = review.ease_factor / 100.0
    
    # In isolation phase, allow immediate practice until mastery is achieved
    if learning_set and learning_set.isolation_phase and new_status not in ["isolation_mastered", "integration_confirmed"]:
        # For isolation phase: much shorter intervals to allow immediate practice
        if review.success_streak < 2:
            # First 2 correct answers: review in 10 minutes
            review.next_due = datetime.utcnow() + timedelta(minutes=10)
        elif review.success_streak < 4:
            # Next 2 correct answers: review in 1 hour  
            review.next_due = datetime.utcnow() + timedelta(hours=1)
        else:
            # After 4+ correct answers: use standard FSRS but cap at 1 day max
            standard_interval = min(interval_days * ease, 1.0)  # Cap at 1 day
            review.next_due = datetime.utcnow() + timedelta(days=standard_interval)
    else:
        # Integration phase or mastered items: use standard FSRS scheduling
        review.next_due = datetime.utcnow() + timedelta(days=interval_days * ease)
