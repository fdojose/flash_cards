...
export default function Learn() {
  ...
  // Helper function to fetch session-level accuracy data
  const fetchSessionAccuracy = async (learningSetId) => {
    try {
      if (!learningSetId) return null;
      console.log('Fetching session accuracy for learning set:', learningSetId);
      const sessionProgress = await apiService.getSessionProgress(learningSetId);
      console.log('Session progress received:', sessionProgress);
      
      const accuracyData = {
        correct_count: sessionProgress.correct_answers || 0,
        total_count: sessionProgress.total_questions || 0,
        accuracy_percentage: sessionProgress.accuracy_percentage || 0
      };
      
      setSessionAccuracyData(accuracyData);
      return accuracyData;
    } catch (error) {
      console.warn('Could not fetch session accuracy:', error);
      return null;
    }
  };
  ...
  const [sessionAccuracyData, setSessionAccuracyData] = useState(null);  // New state for session-level accuracy
  ...
  // Check for session completion when session starts but no flashcard is available
  useEffect(() => {
    ...
    // If we have a session, no flashcard, no loading, and not already complete, then session is complete
    // BUT: Don't mark complete if we're in force review mode (we expect no flashcard initially)
    if (session && !currentFlashcard && !sessionLoading && !sessionComplete && !isForceReviewMode) {
      console.log('Session exists but no flashcard - dataset appears to be completed, trying force review...');
      
      // Hide timer settings and try force review automatically
      setShowTimerSettings(false);
      handleStartSession(datasetId, true, timerSettings.timerMode, timerSettings.timerDuration);
    }
  ...
  const handleTimerSettingsCancel = () => {
    ...
    try {
      ...
      if (!newSession) {
        ...
        // Try to fetch session accuracy data for completed session
        if (datasetId) {
          // Since we don't have a learning set ID, we can't fetch session progress
          // This case might need dataset-level progress instead
          console.log('Session completed without learning set ID - using dataset progress');
        }
      }
    } catch (error) {
